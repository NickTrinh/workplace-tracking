...
def boxes_overlap(box1, box2):
    ...
    x_mid2 = (x_min2 + x_max2)/2
    y_mid2 = (y_min2 + y_max2)/2
    
    return not (x_max1 < (x_min2 + x_mid2)/2 or (x_max2 + x_mid2)/2 < x_min1 or y_max1 < (y_min2 + y_mid2)/2 
                or (y_max2 + y_mid2)/2 < y_min1)
...
while cap.isOpened():
    ...
    if ret:
        ...
        results = model.track(frame, persist=True, tracker='bytetrack.yaml', imgsz=1920, device='cuda:0', conf=0.2)
        ...
        for box, class_id, track_id in zip(boxes, class_ids, track_ids):
            ...
            if direction[track_id] == 'tested':
                cv2.putText(annotated_frame, "Tested", (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
